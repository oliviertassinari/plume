 ...
 *           characteristics at the peer server.
 ...
 *           has more than the supported number of characteristics, then the first found supported
 *           number of characteristics will only be discovered. Also no descriptors other than
 *           Client Characteristic Configuration Descriptors will be searched for at the peer.
 *
 * @note     Presently only one instance of Primary Service can be discovered by this module. If
...
typedef enum
{
    BLE_DB_DISCOVERY_COMPLETE = 1,                                                            /**< Event indicating that the GATT Database discovery is complete. */
 ...
 *
 * @details This structure will be used to identify an instance of this module. For example, there
 *          can be one instance of this module per connection. If there are multiple connections,
 *          that would mean that there are multiple instances of this structure each containing the
 *          information about the GATT Database at the server in that connection.
 */
typedef struct
{
...
/**@brief   Structure containing information need to initialize the DB Discovery module.
 */
typedef struct
{
    ble_srv_error_handler_t        error_handler;                                             /**< Handler that will be called in case of an error condition. */
 ...
 *
 * @param[in]   p_db_discovery_init Pointer to the structure containing initialization information.
 *
 ...
 *
 * @retval      NRF_SUCCESS on successful close.
 */
 ...
 *
 * @details     The application can use this function to register with the module. The application
 *              can use this function to inform which service it is interested in discovering at the
 *              server.
 *
 ...
 * @retval      NRF_ERROR_NULL when a NULL pointer is passed as input.
 * @retval      NRF_ERROR_INVALID_STATE if this function is called without calling the @ref
 *              ble_db_discovery_init.
 * @retval      NRF_ERROR_NOT_SUPPORTED when the total number of applications already registered
 *              with this module is equal to @ref BLE_DB_DISCOVERY_MAX_USERS.
 */
 ...
 *                              started
 *
 * @retval      NRF_SUCCESS if the database discovery was successfully started.
 * @retval      NRF_ERROR_NULL when a NULL pointer is passed as input.
 * @retval      NRF_ERROR_INVALID_STATE if this function is called without calling the @ref
 *              ble_db_discovery_init OR without calling @ref ble_db_discovery_register.
 * @retval      NRF_ERROR_BUSY if a discovery is already in progress for the current connection.
 * @return      This API propagates the error code returned by the SoftDevice API @ref
